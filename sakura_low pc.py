...
def low_pc_jvm_flags(ram_mb):
    """Flags JVM ULTRA AGRESSIFS pour PC faibles (< 8 Go RAM, CPU lent).
    Priorité : démarrer vite, garder la mémoire la plus basse possible,
    zéro freeze GC. Utilise SerialGC sous 1,5 Go (mono-thread, overhead
    minimal) et G1GC au-dessus avec des pauses cibles très courtes.
    """
    xms_mb = max(256, ram_mb // 6)      # heap initial minuscule = démarrage rapide
    if ram_mb <= 1536:
        gc_flags = [
            "-XX:+UseSerialGC",         # GC mono-thread, pas d'overhead sur CPU faible
        ]
    else:
        gc_flags = [
            "-XX:+UseG1GC",
            "-XX:+ParallelRefProcEnabled",
            "-XX:MaxGCPauseMillis=50",  # pauses GC max 50ms au lieu de 200ms
    ...
    return [
        f"-Xms{xms_mb}m", f"-Xmx{ram_mb}m",
        *gc_flags,
        "-XX:CICompilerCount=1",        # 1 thread JIT = moins de CPU au démarrage
        "-XX:+TieredCompilation",
        "-XX:TieredStopAtLevel=1",      # compilation rapide niveau 1 (interp+C1 seulement)
        "-XX:+UseStringDeduplication",  # déduplique les String en mémoire
...
class SakuraLauncher:
    def __init__(self):
        self.root = ctk.CTk()
        self.root.title("Sakura Launcher — Mode PC Faible ⚡")
        ...
        self.ram_mb        = tk.IntVar(value=self._auto_ram_default())
    ...
    @staticmethod
    def _auto_ram_default():
        """Alloue automatiquement 40% de la RAM système, plafonné à 4096 MB
        et avec un minimum de 1024 MB. Sur un PC faible (4 Go total) ça
        donne ~1638 MB — réaliste sans étouffer Windows."""
        total = detect_system_ram_mb()
        if total is None:
            return 2048
        suggested = max(1024, min(4096, int(total * 0.40)))
        # Arrondi au multiple de 512 MB le plus proche
        return (suggested // 512) * 512

    def _check_low_pc_startup(self):
        """Détecte si le PC est faible au démarrage et active automatiquement
        les flags JVM low-PC + affiche un bandeau d'info."""
        total = detect_system_ram_mb()
        if total is None:
            return
        if total < 8192:
            self._low_pc_mode.set(True)
            self._add_log(
                f"PC faible détecté ({total // 1024} Go RAM) — Mode PC Faible activé automatiquement")
            self.root.after(800, lambda: self._add_log(
                "⚡ Flags JVM ultra agressifs appliqués pour maximiser les FPS"))
        else:
            self._low_pc_mode.set(False)
    ...
    def _page_optimisation(self, f):
        ...
        # ── CARTE MODE PC FAIBLE ──────────────────────────────────────────────
        low_card = ctk.CTkFrame(scroll, fg_color="#1a0a2e", corner_radius=10,
                                border_width=2, border_color=ACCENT)
        low_card.pack(fill="x", pady=(0,12))
        header = ctk.CTkFrame(low_card, fg_color="transparent")
        header.pack(fill="x", padx=14, pady=(10,4))
        ctk.CTkLabel(header, text="⚡  MODE PC FAIBLE — ULTRA AGRESSIF",
                     font=ctk.CTkFont(size=13, weight="bold"),
                     text_color=ACCENT2).pack(side="left")
        ctk.CTkSwitch(header, text="Activé", variable=self._low_pc_mode,
                      progress_color=ACCENT, button_color=ACCENT2,
                      font=ctk.CTkFont(size=12), text_color=TEXT).pack(side="right")
        # Preset buttons
        preset_row = ctk.CTkFrame(low_card, fg_color="transparent")
        preset_row.pack(fill="x", padx=14, pady=(0,4))
        presets = [
            ("1 Go  (très faible)", 1024),
            ("2 Go  (faible)", 2048),
            ("3 Go  (moyen-faible)", 3072),
            ("4 Go  (correct)", 4096),
        ]
        for label, val in presets:
            ctk.CTkButton(preset_row, text=label, height=28, width=140,
                          fg_color=CARD2, border_color=ACCENT, border_width=1,
                          text_color=TEXT2, font=ctk.CTkFont(size=11),
                          command=lambda v=val: (self.ram_mb.set(v),
                                                  self._add_log(f"Preset PC faible : {v} MB"),
                                                  self._low_pc_mode.set(True))
                          ).pack(side="left", padx=3)
        # RAM system info
        total_ram = detect_system_ram_mb()
        ram_info = f"RAM système détectée : {total_ram // 1024} Go" if total_ram else "RAM système : inconnue"
        ram_color = RED_C if total_ram and total_ram < 4096 else ORANGE if total_ram and total_ram < 8192 else GREEN
        ctk.CTkLabel(low_card, text=ram_info, text_color=ram_color,
                     font=ctk.CTkFont(size=11, weight="bold")).pack(anchor="w", padx=14, pady=(0,2))
        # JVM flags preview
        flags_low = low_pc_jvm_flags(self.ram_mb.get())
        self._low_flags_preview = ctk.CTkLabel(
            low_card,
            text=" ".join(flags_low[:6]) + " ...",
            font=ctk.CTkFont(size=10, family="Courier"),
            text_color=CYAN, justify="left", wraplength=700)
        self._low_flags_preview.pack(anchor="w", padx=14, pady=(0,8))
        ctk.CTkButton(low_card, text="⚡ Appliquer les flags Ultra-Agressifs",
                      height=34, fg_color=ACCENT, hover_color="#6d28d9",
                      font=ctk.CTkFont(size=12, weight="bold"),
                      command=self._apply_low_pc_jvm).pack(fill="x", padx=14, pady=(0,12))
    ...
    def _launch_current(self):
        ...
        def run():
            try:
                ...
                if self._low_pc_mode.get():
                    jvm_args = low_pc_jvm_flags(ram_xmx)
                else:
    ...
    def _apply_low_pc_jvm(self):
        ram = self.ram_mb.get()
        self._low_pc_mode.set(True)
        flags = low_pc_jvm_flags(ram)
        preview = " ".join(flags[:6]) + " ..."
        try:
            self._low_flags_preview.configure(text=preview)
        except Exception:
            pass
        try:
            self._jvm_preview.configure(text=preview)
        except Exception:
            pass
        self._add_log(f"⚡ Flags JVM Ultra-Agressifs PC Faible appliqués : Xmx{ram}m")
    ...
    def _start_background_optimizer(self):
        """Version ULTRA AGRESSIVE PC FAIBLE — cycle 2s, nettoyage maximal
        du launcher (jamais du système ou d'autres process) :
        - gc.collect(2) + trim_own_memory() à chaque cycle (2s)
        - purge des StringVar orphelines et des PhotoImage inutilisées
        - surveille et rétablit la priorité CPU du jeu (ABOVE_NORMAL)
        - si le PC est très chargé (RAM système > 90%), vide aussi le
          working set du launcher pour libérer de la RAM pour le jeu
        - jamais de HIGH/REALTIME (gèle tout le PC, testé)."""
        def loop():
            cycle = 0
            while True:
                time.sleep(2)
                ...
                try:
                    import gc
                    gc.collect(2)           # collecte génération 2 (objets anciens aussi)
                    trim_own_memory()
                except Exception:
                    pass

                # Toutes les 10 cycles (~20s) : vérif RAM système + trim agressif
                if cycle % 10 == 0:
                    try:
                        import psutil
                        mem = psutil.virtual_memory()
                        if mem.percent > 85 and sys.platform == "win32":
                            import ctypes
                            handle = ctypes.windll.kernel32.GetCurrentProcess()
                            ctypes.windll.psapi.EmptyWorkingSet(handle)
                    except Exception:
    ...
    def _update_stats(self):
        def run():
            ...
            def upd():
                ...
                self.root.after(20000, self._update_stats)
            self.root.after(0, upd)
        threading.Thread(target=run, daemon=True).start()
